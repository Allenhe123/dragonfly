...
bool GetProtoFromASCIIFile(const std::string &file_name,
  ...
  if (file_descriptor < 0) {
    printf( "Failed to open file %s in text mode.\n", file_name.c_str());
    // Failed to open;
    return false;
  }
  ...
  if (!success) {
    printf("Failed to parse file %s as text proto.\n", file_name.c_str());
  }
  delete input;
  close(file_descriptor);
  return success;
...
bool GetProtoFromBinaryFile(const std::string &file_name,
  ...
  if (!input.good()) {
    printf("Failed to open file %s in binary mode.\n", file_name.c_str());
    return false;
  }
  if (!message->ParseFromIstream(&input)) {
    printf("Failed to parse file %s as binary proto.\n", file_name.c_str());
    return false;
  }
  return true;
}
...
bool CopyFile(const std::string &from, const std::string &to) {
  ...
  if (!src) {
    printf("Source path could not be normally opened: %s\n", from.c_str());
    std::string command = "cp -r " + from + " " + to;
    printf("%s\n", command.c_str());
    const int ret = std::system(command.c_str());
    if (ret == 0) {
        printf("Copy success, command returns \n");
      return true;
    } else {
      printf("Copy error, command returns\n");
      return false;
    }
  }
...
bool CopyDir(const std::string &from, const std::string &to) {
  ...
  if (EnsureDirectory(to)) {
    ...
    printf("Cannot create target directory %s\n",  to.c_str());
    ret = false;
  }
  closedir(directory);
  return ret;
